        ...
        {spellDetails && (
          <>
            <Row>
              ...
              <Col>
                <p>
                  <h2>School:</h2> {spellDetails?.school?.name}
                </p>
              </Col>
            </Row>
            ...
            {spellDetails?.dc?.dc_type?.name && (
              <p>
                Save: {spellDetails.dc.dc_type.name} (
                {spellDetails.dc.success_type})
              </p>
            )}
            ...
            {spellDetails?.damage?.damage_at_slot_level && (
              <>
                ...
                <ul>
                  {Object.entries(spellDetails.damage.damage_at_slot_level).map(
                    ([slot, dmg]) => (
                      <li key={slot}>
                        Level {slot}: {dmg}
                      </li>
            ...
            {spellDetails?.heal_at_slot_level && (
              <>
                ...
                <ul>
                  {Object.entries(spellDetails.heal_at_slot_level).map(
                    ([slot, heal]) => (
                      <li key={slot}>
                        Level {slot}: {heal}
                      </li>
            ...
            {spellDetails?.area_of_effect && (
              <p>
                Area: {spellDetails.area_of_effect.size} ft{" "}
            ...
            {spellDetails?.subclasses?.length > 0 && (
              <p>
                Subclasses:{" "}
                {spellDetails.subclasses.map((s) => s.name).join(", ")}
              </p>
            )}
